...
class notMNIST(VisionDataset):
    """`MNIST <http://yaroslavvb.com/upload/notMNIST/>`_ Dataset.
    Args:
        root (string): Root directory of dataset where ``notMNIST/processed/training.pt``
            and  ``notMNIST/processed/test.pt`` exist.
        train (bool, optional): If True, creates dataset from ``training.pt``,
            otherwise from ``test.pt``.
        download (bool, optional): If true, downloads the dataset from the internet and
            puts it in root directory. If dataset is already downloaded, it is not
    ...
    resources = [
                 {"url":"http://yaroslavvb.com/upload/notMNIST/notMNIST_large.tar.gz", "md5": None, "filename": "notMNIST_large.tar.gz", "name":"notMNIST_large"},
                 {"url":"http://yaroslavvb.com/upload/notMNIST/notMNIST_small.tar.gz", "md5": None, "filename": "notMNIST_small.tar.gz", "name":"notMNIST_small"},
    ]
    ...
    # 난 굳이 이걸 왜 프로퍼티로 작성했는지 모르겠음 나라면 싸그리 풀거나 변수로 선언할 것이다. 이걸 숨겨서 뭐할건데?
    @property
    def raw_folder(self) -> str:
        return os.path.join(self.root, self.__class__.__name__, 'raw')
    ...
    def download(self) -> None:
        ...
        for resource in self.resources:
            url, filename, md5 = resource["url"], resource["filename"], resource["md5"]
            download_and_extract_archive(url, download_root=self.raw_folder, filename=filename, md5=md5)

        # process and save as torch files
        print('Processing...')

        training_set = read_image_and_label_file(os.path.join(self.raw_folder, 'notMNIST_large'))
        test_set = read_image_and_label_file(os.path.join(self.raw_folder, 'notMNIST_small'))
    ...
    def extra_repr(self) -> str:
        return "Split: {}".format("Train" if self.train is True else "Test") # f 스트링으로 쓰자.


def read_image_and_label_file(path: str) -> torch.Tensor:
  result_img = []
  labels = []
  class_folder = os.listdir(path)
  class_to_idx = {_class: i for i, _class in enumerate( sorted(class_folder))}
  error_file_name = []

  for class_ in class_folder:
    class_path = os.path.join(path, class_)

    for data in os.listdir(class_path):
      try:
        img = Image.open(os.path.join(path, class_, data))
        img_np = np.array(img)
        result_img.append(img_np)

        labels.append(class_to_idx[class_])
      except:
        error_file_name.append(os.path.join(path, class_, data))
  ...
  assert(result_img.ndimension() == 3)
